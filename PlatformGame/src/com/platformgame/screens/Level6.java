...
public class Level6 extends BaseLevel {
	...
	public Level6(PlatformGame game) {
		super(game);
	...
	public void show() {
		level = 6;
		if(game.manager.isLoaded("maps/level6.tmx")){
			TiledMap map = game.manager.get("maps/level6.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelSixHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelSixStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelSixHighScore(currenCrystals);
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelSixStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelSixStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelSixStars(3);
			}
		}
