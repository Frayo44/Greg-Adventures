...
public class Level9 extends BaseLevel {
		...
		public Level9(PlatformGame game) {
			super(game);
		...
		public void show() {
			level = 9;
			if(game.manager.isLoaded("maps/level9.tmx")){
				TiledMap map = game.manager.get("maps/level9.tmx");
				loadMap(map);
			} else{
		...
		protected void initLevelCompletedUI() {
			...
			int besthighscore = game.getPreferencesManager().getLevelNineHighScore();
			int totalCryss = player.getTotalCrystals();
			int curStars = game.getPreferencesManager().getLevelNineStars();

			if (currenCrystals >= besthighscore) {
				game.getPreferencesManager().setLevelNineHighScore(currenCrystals);
				besthighscore = game.getPreferencesManager().getLevelNineHighScore();
			}
			...
			if (currenCrystals <= totalCryss / 2) {
				...
				if (curStars < 1) {
					game.getPreferencesManager().setLevelNineStars(1);
				}
				...
				stage.addActor(nextLevelImage);
				if (curStars < 2) {
					game.getPreferencesManager().setLevelNineStars(2);
				}
			} else {
				Drawable pauseDrawable = new TextureRegionDrawable(
						new TextureRegion(new Texture(
				...
				if (curStars <= 3) {
					game.getPreferencesManager().setLevelNineStars(3);
				}
			}
		...
		public void dispose() {
			...
			//game.manager.load("images/rope3.png", Texture.class);
...
//			game.manager.load("images/background3.png", Texture.class);
			...
			game.manager.unload("images/background3.png");
			//game.manager.unload("images/rope9.png");
		//	game.manager.unload("images/background3.png");
		}
