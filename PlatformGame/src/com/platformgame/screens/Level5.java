...
public class Level5 extends BaseLevel {
	...
	public Level5(PlatformGame game) {
		super(game);
	...
	public void show() {
		level = 5;
		if(game.manager.isLoaded("maps/level5.tmx")){
			TiledMap map = game.manager.get("maps/level5.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelFiveHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelFiveStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelFiveHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getLevelFiveHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelFiveStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelFiveStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelFiveStars(3);
			}
		}
