...
public class Level4 extends BaseLevel {
	...
	public Level4(PlatformGame game) {
		super(game);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelFourHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelFourStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelFourHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getLevelFourHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelFourStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelFourStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelFourStars(3);
			}
		}
