...
import com.platforngame.handlers.BaseLevel.GameState;

public class Level3 extends BaseLevel {

	public Level3(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if(game.manager.isLoaded("maps/level3.tmx")){
			TiledMap map = game.manager.get("maps/level3.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelThreeHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelThreeStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelThreeHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getLevelThreeHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelThreeStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelThreeStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelThreeStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("animations/dragonfly.png");
		game.manager.unload("maps/level3.tmx");
		game.manager.unload("images/backgroundnight.png");
		game.manager.unload("images/tree.png");
		game.manager.unload("images/log.png");
	}
