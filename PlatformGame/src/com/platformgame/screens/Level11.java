...
public class Level11 extends BaseLevel {
	...
	public Level11(PlatformGame game) {
		super(game);
	...
	public void show() {
		level = 11;
		if(game.manager.isLoaded("maps/level11.tmx")){
			TiledMap map = game.manager.get("maps/level11.tmx");
			loadMap(map);
		} else{
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelElevenHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelElevenStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelElevenHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getLevelElevenHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelElevenStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelElevenStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelElevenStars(3);
			}
		}
