...
public class Level10 extends BaseLevel {
	...
	public Level10(PlatformGame game) {
		super(game);
	...
	public void show() {
		level = 10;
		if(game.manager.isLoaded("maps/level10.tmx")){
			TiledMap map = game.manager.get("maps/level10.tmx");
			loadMap(map);
		} else{
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelTenHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelTenStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelTenHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getLevelTenHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelTenStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelTenStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelTenStars(3);
			}
		}
