...
public class Level1 extends BaseLevel {
	...
	public Level1(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if (game.manager.isLoaded("maps/level1.tmx")) {
			TiledMap map = game.manager.get("maps/level1.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelOneHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelOneStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelOneHighScore(currenCrystals);
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelOneStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelOneStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelOneStars(3);
			}
		}
		...
		if (game.actionResolver.getSignedInGPGS()) {
			int total_upper = (int) total_amount * 1;
