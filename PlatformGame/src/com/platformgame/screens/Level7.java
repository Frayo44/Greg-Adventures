...
public class Level7 extends BaseLevel {
	...
	public Level7(PlatformGame game) {
		super(game);
	...
	public void show() {
		level = 7;
		if(game.manager.isLoaded("maps/level7.tmx")){
			TiledMap map = game.manager.get("maps/level7.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getLevelSevenHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getLevelSevenStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setLevelSevenHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getLevelSevenHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setLevelSevenStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setLevelSevenStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setLevelSevenStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("images/background4.png");
