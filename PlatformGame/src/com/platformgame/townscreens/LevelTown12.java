...
public class LevelTown12 extends  BaseLevelTown {

	public LevelTown12(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if (game.manager.isLoaded("maps/leveltown12.tmx")) {
			TiledMap map = game.manager.get("maps/leveltown12.tmx");
			loadMap(map);
		} else {
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDTwelveHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDTwelveStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDTwelveHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getWORLDTwelveHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDTwelveStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDTwelveStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDTwelveStars(3);
			}
		}
		...
		if (game.actionResolver.getSignedInGPGS()) {
			int total_upper = (int) (total_amount + currenCrystals * 1f);
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown12.tmx");
