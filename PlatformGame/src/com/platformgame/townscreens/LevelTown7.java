...
public class LevelTown7 extends BaseLevelTown {

	public LevelTown7(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if (game.manager.isLoaded("maps/leveltown7.tmx")) {
			TiledMap map = game.manager.get("maps/leveltown7.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDSevenHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDSevenStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDSevenHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getWORLDSevenHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDSevenStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDSevenStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDSevenStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown7.tmx");
