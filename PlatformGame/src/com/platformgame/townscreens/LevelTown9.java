...
public class LevelTown9 extends  BaseLevelTown {

	public LevelTown9(PlatformGame game) {
		super(game);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDNineHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDNineStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDNineHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getWORLDNineHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDNineStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDNineStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDNineStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown9.tmx");
