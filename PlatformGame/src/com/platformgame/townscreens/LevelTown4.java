...
import com.platformgame.townscreens.BaseLevelTown.GameState;

public class LevelTown4 extends BaseLevelTown {

	public LevelTown4(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		game.manager.load("maps/levelTown4.tmx", TiledMap.class);
		if (game.manager.isLoaded("maps/levelTown4.tmx")) {
			TiledMap map = game.manager.get("maps/levelTown4.tmx");
			loadMap(map);
			System.out.println("DFGDF");
		} else {
			game.manager.setLoader(TiledMap.class, new TmxMapLoader(
					new InternalFileHandleResolver()));
			game.manager.load("maps/levelTown3.tmx", TiledMap.class);
			if (game.manager.isLoaded("maps/levelTown3.tmx")) {
				TiledMap map = game.manager.get("maps/levelTown3.tmx");
				loadMap(map);
			}
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDFourHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDFourStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDFourHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getWORLDFourHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDFourStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDFourStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDFourStars(3);
			}
		}
		...
		if (game.actionResolver.getSignedInGPGS()) {
			int total_upper = (int) (total_amount + currenCrystals * 0.2f);
	...
	public void dispose() {
		...
		game.manager.unload("images/woodplatform.png");
	}
