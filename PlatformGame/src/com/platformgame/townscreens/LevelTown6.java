...
public class LevelTown6 extends BaseLevelTown {

	public LevelTown6(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if (game.manager.isLoaded("maps/leveltown6.tmx")) {
			TiledMap map = game.manager.get("maps/leveltown6.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDSixHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDSixStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDSixHighScore(currenCrystals);
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDSixStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDSixStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDSixStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown6.tmx");
