...
public class LevelTown10 extends  BaseLevelTown {

	public LevelTown10(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if (game.manager.isLoaded("maps/leveltown10.tmx")) {
			TiledMap map = game.manager.get("maps/leveltown10.tmx");
			loadMap(map);
		} else {
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDTenHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDTenStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDTenHighScore(currenCrystals);
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDTenStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDTenStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDTenStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown10.tmx");
