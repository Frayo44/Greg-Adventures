...
public class LevelTown8 extends  BaseLevelTown {

	public LevelTown8(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if (game.manager.isLoaded("maps/leveltown8.tmx")) {
			TiledMap map = game.manager.get("maps/leveltown8.tmx");
			loadMap(map);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDEightHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDEightStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDEightHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getWORLDEightHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDEightStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDEightStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDEightStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown8.tmx");
