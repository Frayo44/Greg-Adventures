...
public class LevelTown3 extends BaseLevelTown {

	public LevelTown3(PlatformGame game) {
		super(game);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDThreeHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDThreeStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDThreeHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getWORLDThreeHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDThreeStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDThreeStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDThreeStars(3);
			}
		}
