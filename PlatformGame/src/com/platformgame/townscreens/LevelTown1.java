...
public class LevelTown1 extends BaseLevelTown {

	public LevelTown1(PlatformGame game) {
		super(game);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDOneHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDOneStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDOneHighScore(currenCrystals);
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDOneStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDOneStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDOneStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown1.tmx");
