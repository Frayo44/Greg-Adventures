...
public class LevelTown2 extends BaseLevelTown {

	public LevelTown2(PlatformGame game) {
		super(game);
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getWORLDTwoHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getWORLDTwoStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setWORLDTwoHighScore(currenCrystals);
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setWORLDTwoStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setWORLDTwoStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setWORLDTwoStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/leveltown2.tmx");
