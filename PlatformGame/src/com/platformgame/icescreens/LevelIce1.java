...
public class LevelIce1 extends BaseLevelIce {

	public LevelIce1(PlatformGame game) {
		super(game);
	...
	public void show() {
		...
		if (game.manager.isLoaded("maps/levelice1.tmx")) {
			TiledMap map = game.manager.get("maps/levelice1.tmx");
			loadMap(map);
			System.out.println("DFGDF");
		} else {
			game.manager.setLoader(TiledMap.class, new TmxMapLoader(
					new InternalFileHandleResolver()));
			game.manager.load("maps/leveltown1.tmx", TiledMap.class);
			if (game.manager.isLoaded("maps/leveltown1.tmx")) {
				TiledMap map = game.manager.get("maps/leveltown1.tmx");
				loadMap(map);
			}
	...
	protected void initLevelCompletedUI() {
		...
		int besthighscore = game.getPreferencesManager().getICEOneHighScore();
		int totalCryss = player.getTotalCrystals();
		int curStars = game.getPreferencesManager().getICEOneStars();

		if (currenCrystals >= besthighscore) {
			game.getPreferencesManager().setICEOneHighScore(currenCrystals);
			besthighscore = game.getPreferencesManager().getICEOneHighScore();
		}
		...
		if (currenCrystals <= totalCryss / 2) {
			...
			if (curStars < 1) {
				game.getPreferencesManager().setICEOneStars(1);
			}
			...
			stage.addActor(nextLevelImage);
			if (curStars < 2) {
				game.getPreferencesManager().setICEOneStars(2);
			}
		} else {
			Drawable pauseDrawable = new TextureRegionDrawable(
					new TextureRegion(new Texture(
			...
			if (curStars <= 3) {
				game.getPreferencesManager().setICEOneStars(3);
			}
		}
	...
	public void dispose() {
		...
		game.manager.unload("maps/levelice1.tmx");
