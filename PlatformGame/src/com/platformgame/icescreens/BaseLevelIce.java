...
public class BaseLevelIce extends AbstractScreen {
	...
	Table tableLabel;
	private Array<Vector2> positionsVerticalCutters;
	// priavte Array[Vector2] positionsVerticalCutters;
	Body bottleModel;
	...
	MapLayer layer2;
	int sideCutters = 0; //0// 0 - left // 1 - right
	Table tablego;
	...
	public BaseLevelIce(PlatformGame game) {
		super(game);
	...
	private Array<Rock> booms;
	private Array<Rock> shields;
	private Array<Rock> rocks;
	private Array<Rock> madBalls;
	...
	private Array<Rock> cuttersVertical;
	private Array<Rock> movingobj;
	...
	public void update(float dt) {
		...
		Array<Body> bodiesBooms = cl.getBodiesToRemoveBooms();
		for (int i = 0; i < bodiesBooms.size; i++) {
			Body b = bodiesBooms.get(i);
			booms.removeValue((Rock) b.getUserData(), true);
			world.destroyBody(b);
			if ((cl.numLifes >= 0)) {
				cl.numLifes -= 1;
			//	cl.numLifesCurr -= 1;
	...
	public void render(float dt) {
		...
		if (cuttersVertical != null) {
			for (int i = 0; i < cuttersVertical.size; i++) {
				cuttersVertical.get(i).render(sb);
				cuttersVertical.get(i).getBody().setGravityScale(0);
				if (i % 2 == 1) {
					// cuttersVertical.get(i).getBody().setLinearVelocity(1, 0);
				} else {
					// cuttersVertical.get(i).getBody().setLinearVelocity(-1,
					// 0);
				}

				if (cuttersVertical.get(i).getBody().getLinearVelocity().x >= 0) {
					if (cuttersVertical.get(i).getBody().getLinearVelocity().x != 1.7f)
						cuttersVertical.get(i).getBody()
								.setLinearVelocity(1.7f, 0);
					sideCutters = 0;
				} else {
					sideCutters = 1;
					if (cuttersVertical.get(i).getBody().getLinearVelocity().x != -1.7f)
		...
		if (movingobj != null) {
			for (int i = 0; i < movingobj.size; i++) {
				movingobj.get(i).render(sb);
				movingobj.get(i).getBody().setGravityScale(0);
//				movingobj.get(i).getBody().linVelLoc.y = 0;
//				movingobj.get(i).getBody().linVelWorld.y = 0;
//				movingobj.get(i).getBody().setAngularVelocity(0);
				movingobj.get(i).getBody().setLinearVelocity(movingobj.get(i).getBody().getLinearVelocity().x,0);
				//movingobj.get(i).getBody().
				if(movingobj.get(i).getBody().getLinearVelocity().y < 0){
					//continue;
					//movingobj.get(i).getBody().setLinearVelocity(movingobj.get(i).getBody().getLinearVelocity().x, movingobj.get(i).getBody().getLinearVelocity().y - 0.08f);
				}
			...
			//	movingobj.get(i).getBody().setTransform(movingobj.get(i).getBody().getLinearVelocity().x, 0, 0);
				//movingobj.get(i).getBody().getPosition().set(new Vector2(movingobj.get(i).getBody().getPosition().x, positionsVerticalCutters.get(i).y));//, 0.0f);
				...
				if (movingobj.get(i).getBody().getLinearVelocity().x >= 0) {
					if (movingobj.get(i).getBody().getLinearVelocity().x != 1.7f)
						movingobj.get(i).getBody()
								.setLinearVelocity(1.7f, 0);
					//sideCutters = 0;
				} else {
		...
		if (rollingSpikes != null) {
			for (int i = 0; i < rollingSpikes.size; i++) {
				...
				// body->GetInertia() * (desiredAngularVelocity -
				// body->getAngularVelocity() ) / time;
				Vector2 position = rollingSpikes.get(i).getBody().getPosition();
				rollingSpikes.get(i).getBody().setTransform(position, degrees);// .applyTorque(degrees,
																				// true);
		...
		for (int i = 0; i < madBalls.size; i++) {
				if (madBalls.get(i).getBody().getLinearVelocity().y >= 0) {
					if (madBalls.get(i).getBody().getLinearVelocity().y != 1.5f)
						madBalls.get(i)
								.getBody()
								.setLinearVelocity(
										0,
	...
	public void show() {
		...
		Texture bgs = null;
		if (game.manager.isLoaded("images/backgroundice.png")) {
			bgs =// new Texture(Gdx.files.internal("images/backgroundice.png"));
			   game.manager.get("images/backgroundice.png");
		} else {

			game.manager.load("images/backgroundice.png", Texture.class);
			if (game.manager.update())
				bgs = //new Texture(
						//Gdx.files.internal("images/backgroundice.png"));
			 game.manager.get("images/backgroundice.png");
		}
	...
	private void createAll() {
		...
		// create verical cutters
		createCuttersVertical();
		
		// create moving obj
		if (tileMap.getLayers().get("movingobj") != null)
		createMovingObj();
...
//				createAxes(13);
	...
	protected void initVars() {
		...
		if (cuttersVertical != null) {
			cuttersVertical.clear();
			cuttersVertical = null;
		}
	...
	private void createMadBalls() {
		/*
		 * All the rocks int the game, spikes rock, slashes rock and regular
		 * rock and etc.
		 */
		madBalls = new Array<Rock>();
		
		MapLayer layer = tileMap.getLayers().get("madballs");
		...
		for (MapObject mo : layer.getObjects()) {
			...
			Rock c = new Rock(body, null, false);
			c.spr.setScale(0.7f);
			Texture texture =// new Texture(Gdx.files.internal("images/slashesrock.png"));
					game.manager.get("images/slashesrock.png");
			c.spr.setTexture(texture);
			madBalls.add(c);
			body.setUserData(c);
	...
	private void createGrabges() {
		...
		for (MapObject mo : layer.getObjects()) {
			...
			cshape.setAsBox(35 / PPM, 15 / PPM);

			fdef.shape = cshape;
			fdef.isSensor = false;
			fdef.density = 7000.1f;
			fdef.friction = 1f;
			fdef.restitution = 0.1f;
			fdef.filter.categoryBits = B2DVars.BIT_PLAYER;
			fdef.filter.maskBits = B2DVars.BIT_PLAYER | B2DVars.BIT_RED
			...
			Rock c = new Rock(body, null, false);
			c.spr.setScale(1f, 0.7f);
			Texture texture =
					//new Texture(
					//Gdx.files.internal("images/bolltree.png"));
					game.manager.get("images/bolltree.png");
	...
	private void createEnemies() {
		...
		for (MapObject mo : layer.getObjects()) {
			...
			fdef.filter.categoryBits = B2DVars.BIT_BALLS;
			fdef.filter.maskBits = B2DVars.BIT_PLAYER | B2DVars.BIT_RED
					| B2DVars.BIT_CRYSTAL | B2DVars.BIT_ROPE | B2DVars.BIT_CAR;
			...
			if (level == 1) {
				if (i <= 3) {
					Texture tex = //new Texture(
							//Gdx.files.internal("animations/penguin.png"));
					 game.manager.get("animations/penguin.png");
	...
	private void createPointsRope() {
		...
		for (MapObject mo : layer.getObjects()) {
			...
			if (level != 10 && level != 9 && level != 11 && level != 12) {
				...
				Texture text = PlatformGame.manager.get("images/swingwood.png");
	...
	private void createCuttersVertical() {// ), int rockIndex) {

		// Texture tex = game.manager.get("images/woodpole.png");
		//positionsVerticalCutters = new Array<Vector2>();
		cuttersVertical = new Array<Rock>();

		MapLayer layer = tileMap.getLayers().get("cutters");
		...
		for (MapObject mo : layer.getObjects()) {
			...
			ebody = world.createBody(bdef);
			ebody.createFixture(fdef).setUserData("ropefire");
			// ebody.setFixedRotation(true);
			ebody.setLinearVelocity(10, 0);
			// ebody.setAngularDamping(100000);
			ebody.setFixedRotation(true);
			...
			if (level != 10 && level != 9 && level != 11 && level != 12) {
				c = new Rock(ebody, null, false);
				Texture text = PlatformGame.manager.get("images/spikesplatformvert.png");
						//new Texture(
						//Gdx.files.internal("images/spikesplatformvert.png"));
				// PlatformGame.manager.get("images/spikesplatform.png");
				c.spr.setTexture(text);
				c.spr.setScale(0.45f, 2.00f);
				// c.spr.setRotation(90);
				// c.spr.setScale(0.3f);
			}
				...
				c.spr.setTexture(text);
				c.spr.setScale(0.25f, 1.99f);
				if (cuttersVertical.size == 6 && level != 12) {
					c.spr.setScale(0.01f);
				}
			}

			cuttersVertical.add(c);
			...
			// ebody.setUserData(c);
		//	positionsVerticalCutters.add(ebody.getPosition());
			pshape.dispose();
		}

	}

	private void createMovingObj() {// ), int rockIndex) {

		// Texture tex = game.manager.get("images/woodpole.png");
		positionsVerticalCutters = new Array<Vector2>();
		movingobj = new Array<Rock>();

		MapLayer layer = tileMap.getLayers().get("movingobj");
		...
		for (MapObject mo : layer.getObjects()) {
			...
			ebody = world.createBody(bdef);
			ebody.createFixture(fdef).setUserData("ropefuire");
			// ebody.setFixedRotation(true);
		//	ebody.setLinearVelocity(10, 0);
			// ebody.setAngularDamping(100000);
			ebody.setFixedRotation(true);
			...
			if (level != 10 && level != 9 && level != 11 && level != 12) {
				c = new Rock(ebody, null, false);
				Texture text = PlatformGame.manager.get("images/swingwoodhorz.png");
						//new Texture(
						//Gdx.files.internal("images/spikesplatformvert.png"));
				// PlatformGame.manager.get("images/spikesplatform.png");
				c.spr.setTexture(text);
				c.spr.setScale(1.50f, 0.40f);
				// c.spr.setRotation(90);
				// c.spr.setScale(0.3f);
			}
...
//			fdef.filter.maskBits = B2DVars.BIT_PLAYER;
//			fdef.isSensor = false;
//			ebody.createFixture(fdef).setUserData("ropefisdre");
//
//			// create foot sensor
//			pshape.setAsBox(2 / PPM, 2 / PPM, new Vector2(0, -1 / PPM), 0);
//
...
//			fdef.filter.maskBits = B2DVars.BIT_PLAYER | B2DVars.BIT_RED;
//			fdef.isSensor = true;
//			ebody.createFixture(fdef).setUserData("sfsdad");
	...
	private void createRollingPlatfomrs(int index) {
		...
		body.setLinearVelocity(0, 0f);
		Rock c = new Rock(body, null, false);
		Texture texte =  game.manager.get("images/spikesplatform.png");
		c.spr.setTexture(texte);
	...
	private void createAxes(int index) {
		...
		// /bottleSprite = new Sprite(new
		// Texture(Gdx.files.internal("images/axe.png")));
		...
		BodyEditorLoader loader = new BodyEditorLoader(
				Gdx.files.internal("physics/axe1.json"));
		...
		c.spr.setOrigin(c.spr.getOriginX() - 1f, c.spr.getOriginY() - 34);// .setPosition(c.spr.getX(),
																			// c.spr.getY()
																			// +
																			// 10);
	...
	public void createBooms() {
		booms = new Array<Rock>();

		MapLayer layer = tileMap.getLayers().get("booms");
		...
		for (MapObject mo : layer.getObjects()) {
			...
			Body body = world.createBody(bdef);
			body.createFixture(fdef).setUserData("booms");
			body.setLinearVelocity(0, 0f);
			Texture boomTex = //game.manager.get("images/boom.png");
			  new
			 Texture(Gdx.files.internal("images/bomb.png"));
			// game.manager.get("images/heart.png");
			Rock c = new Rock(body, null, false); // FIX
			c.spr.setTexture(boomTex);
			c.spr.setScale(0.37f);
			booms.add(c);
